		...
		for (int loopCounter = 0; loopCounter < 100; loopCounter++) {
			...
			if (newScore > highScore) {
				...
				for (int i = 0; i < students.size(); i++) {
					s = students.get(i);
					bestStudents.add(
							new Student(s.getID(), s.getRequestedCourses(), s.getAlternateCourses(), s.getTimeTable()));
				}
			}
			/*
			 * for (int i = 0; i < courses.size(); i++) { if
			 * (courses.get(i).getCode().equals("ACAL-12---")) {
			 * System.out.println("num of courses: " + courses.size() + " num of students: "
			 * + students.size() + " requested students: " +
			 * courses.get(i).getRequestedStudents().size() + " actual students: " +
		...
		for (ArrayList<CourseSection> courses : bestTable.getTimetable()) {
			for (CourseSection c : courses) {
				if (c.getPairedCourse() != null) {
					System.out.println(c.getCourse().getCode() + " paired with "
							+ c.getPairedCourse().getCourse().getCode() + "!");
				}
			}
		}
...
//			highIndices = (int)(Math.random() * coursesToCheck.size());
			...
			// System.out.println("checkpoint. courses to check size: " +
			// coursesToCheck.size());
			// Sort preferences
			for (int i = 0; i < prefsInOrder.length; i++) {
				...
				for (int j = i + 1; j < prefsInOrder.length; j++) {
					if (preferences[prefsInOrder[j]] > highPref
							|| (preferences[prefsInOrder[j]] == highPref && (int) (Math.random() * 2) != 1)) {
						highIndex = j;
...
//			}
			...
			if (coursesToCheck.get(highIndices).getNumSections() <= 1) {
				...
				while (coursesToCheck.get(highIndices).getSection(coursesToCheck.get(highIndices).getNumSections() - 1)
						.getBlock() == -1) {
					for (int i = 0; i < prefsInOrder.length; i++) {
	...
	public static void generateStudentPreferences() {
		for (Student s : students) {
			...
			for (Course c : s.getRequestedCourses()) {
				ArrayList<Integer> slotPreferences = s.getEmptySlots(); // Slots that this student (s) would like this
																		// course (c) to be in
				...
				// Check if this courses has any sequencing rules that interact with this
				// student's other courses
					...
					if (s.getRequestedCourses().contains(cSeq)) {
						...
						for (int i = 0; i < slotPreferences.size(); i++) {
							if (slotPreferences.get(i) > 4) {
								slotPreferences = (ArrayList<Integer>) copyArrayListPortion(slotPreferences, i,
										slotPreferences.size());
							}
						}
					} // if
				} // for cSeq
								...
								if (i > 3) {
									for (int j = 0; j < slotPreferences.size(); j++) {
										if (slotPreferences.get(j) > 4)
											slotPreferences = (ArrayList<Integer>) copyArrayListPortion(slotPreferences,
													j, slotPreferences.size());
									}
								} else {
									for (int j = 0; j < slotPreferences.size(); j++) {
										if (slotPreferences.get(j) > 4)
		...
		for (int i = courses.size() - 1; i >= 0; i--) {
			for (int k = 0; k < courses.get(i).getNumSections(); k++) {
				...
				if ((double) courses.get(i).getSection(k).getNumStudents() / courses.get(i).getCapacity() < .5) {
					// Loop through course sections in this block to see if any can be run as a
					// split
...
//							System.out.println(courses.get(i).getCode() + " split with " + c.getCourse().getCode() + "!");
	...
	// returns the metrics all requested courses placed
	/*
	 * public static double genReqCourseMetrics(ArrayList<Student> stuList) { return
	 * (double)getCoursesPlaced(stuList) / getCoursesRequested(stuList); } //
	 * genReqCourseMetrics
	 */
	/*
	 * public static String reqCoursePlaced(ArrayList<Student> stuList) { int
	 * totalReqCourses = 0; int totalPlacedReqCourses = 0;
	 * 
	 * for (Student s : stuList) { totalReqCourses +=
	 * s.getRequestedCourses().size(); for (Course reqCourse :
	 * s.getRequestedCourses()) { for (CourseSection actualCourse :
	 * s.getTimeTable().getAllCourseSections()) { // check if a given actualCourse
	 * was requested if
	 * (reqCourse.getCode().equals(actualCourse.getCourse().getCode())) {
	 * totalPlacedReqCourses++; break; } // if } // for (student s' requested
	 * courses) } // for (student s' actual courses) } // for (student)
	 * 
	...
	/*
	 * public static double genSufficientReqMetrics(ArrayList<Student> stuList, int
	 * lowerLim, boolean includeAlts) { int numFullReqStudents = 0;
	 * 
	 * for (Student s : stuList) { int numReqCoursesGiven = 0;
	 * 
	 * ArrayList<Course> reqAndAltCourses = s.getRequestedCourses(); if () { for
	 * (Course alt : s.getAlternateCourses()) { reqAndAltCourses.add(alt); } }
	 * 
	 * 
	 * for (Course reqCourse : s.getRequestedCourses()) { for (CourseSection
	 * actualCourse : s.getTimeTable().getAllCourseSections()) { if
	 * (actualCourse.getCourse().getCode().equals(reqCourse.getCode())) {
	 * numReqCoursesGiven++; break; } } // for (student s' requested courses) } //
	 * for (student s' actual courses) if (numReqCoursesGiven >= lowerLim &&
	 * numReqCoursesGiven < 9) { if (lowerLim == 8) { System.out.println(s.getID());
	 * System.out.println(s.getTimeTable()); } numFullReqStudents++; } // if } //
	 * for (students)
	 * 
		...
		for (int i = 0; i < lines; i++) {
			if (data[i].length == 9) {
				course = new Course(data[i][1], data[i][0], data[i][7], data[i][8],
						(data[i][0].charAt(data[i][0].length() - 1) == 'L'));
