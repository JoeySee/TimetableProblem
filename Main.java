...
//			highIndices = (int)(Math.random() * coursesToCheck.size());
			...
			for(int i = 0; i < prefsInOrder.length; i++) {
				...
				highPref = preferences[prefsInOrder[i]];
				int highIndex = i;
...
//			}
			...
			if(coursesToCheck.get(highIndices).getNumSections() <= 1) {
				...
				if(coursesToCheck.get(highIndices).isLinear()) {
//					coursesToCheck.get(highIndices).addCourseToBlock(solution, 0);
//					coursesToCheck.get(highIndices).addCourseToBlock(solution, 7);
